...
| **Tiered Hybrid** ⭐ | `combo2` | A tiny human-readable **summary** by default; the full binary tree on error or when asked | Looks like today's request diagnostics, costs almost nothing on the happy path, and never loses detail when something breaks. **Recommended default.** |
...
If you only remember one thing: **the Tiered Hybrid's default output is a small summary that looks
like today's diagnostics; the Binary Span Tree is the compact full-detail tree behind it; the Eager
JSON Baseline is the always-verbose design we compare against.**
...
**Pursue the Binary Span Tree + Tiered Hybrid together; keep the Eager JSON Baseline as the
OTel/baseline path.** The first two share one binary wire format (`AZD1`), so they are complementary
rather than competing:

- **Tiered Hybrid** is the recommended **default customer-facing** output. Its summary is
...
- **Binary Span Tree** (arena → binary) is the recommended **detailed tier** (the Tiered Hybrid
  already reuses it). Full fidelity, **free to drop on success**, **1.4–4.2× smaller** than baseline
  JSON, decoded by a shared `diag-decode` tool.
- **Eager JSON Baseline** (tracing-native) stays as the baseline and the natural **OpenTelemetry
  export** path, not the default — it pays full eager cost on every call (**~12.9 µs** on the happy
  path, ~14× the Tiered Hybrid default) and emits the largest JSON.

**Headline numbers (median, release, Windows x86_64):**

| Metric | Eager JSON Baseline | Binary Span Tree | Tiered Hybrid (default summary) |
|---|--:|--:|--:|
...
1. **Canonical capture model & default verbosity** — adopt the Tiered Hybrid's summary as the default
   with the Binary Span Tree's binary as the on-demand/on-error detail tier? (Recommended: yes.)
...
| **Binary Span Tree** (`combo1`) | A2/A3 + C2 + D2 + D4 | Primary: full-fidelity, cheap, small |
| **Tiered Hybrid** (`combo2`) | A3 + B3 + C3 + D3 | Primary: customer- & migration-friendly default |
...
|---|--:|---|
| Eager JSON Baseline (eager) | 12,853 | collect+construct+serialize, **always** |
| Binary Span Tree (arena, drop on success) | 2,042 | collect + free drop (774 ns); **no serialize** |
| Tiered Hybrid (summary default) | **891** | collect + reduce + summary JSON |

**Reading the numbers.**
- **Size grows with fan-out for JSON, not for the summary.** The Eager JSON Baseline balloons to
  3,464 B at S4×25; the Tiered Hybrid's summary stays flat at 304 B (it collapses 25 children to a
  `child_count`) → 11.4×.
- **Happy path:** the Tiered Hybrid's default is ~14× cheaper than the eager baseline; the Binary
  Span Tree's drop-on-success is ~6× cheaper.
- **DEFLATE is the cost on big blobs.** The binary `serialize` (Binary Span Tree, and the Tiered
  Hybrid's detailed tier) jumps from ~1.2 µs (S1–S3, uncompressed) to ~35–47 µs at S4 once the
  payload crosses the 512 B auto-compress threshold. That cost is **off the hot path** (only on
  error/verbose) and the threshold is the single tuning knob.
...
### The three designs vs .NET — how the same operation looks
...
| .NET field | Eager JSON Baseline | Binary Span Tree (decoded node) | Tiered Hybrid summary |
|---|---|---|---|
...
- .NET's **`Summary` block is conceptually the Tiered Hybrid's entire default output** — a flat,
  aggregatable roll-up. That is exactly why the Tiered Hybrid is the migration-friendly path and .NET
  is the natural second adopter: a .NET reader already thinks in "summary + drill-down".
- .NET's **full nested tree is conceptually the Binary Span Tree's decoded blob** — the same
  information, but the Binary Span Tree ships it as a compact binary you only materialize on error or
  on demand, instead of building the whole tree every time.
- **The Eager JSON Baseline sits in between**: one eager JSON object per call — easiest to map onto
  OpenTelemetry spans, but the bulkiest and always-on, much like emitting the full .NET tree
  unconditionally.
- Net: today's .NET experience is "always build the big tree, with a summary on top." The
  recommended Rust direction (Tiered Hybrid + Binary Span Tree) keeps the **summary as the cheap
  default** and makes the **big tree an opt-in, compact, on-error artifact**.
...
> **Tiered Hybrid** = "cheap summary by default, full detail on demand".

| Criterion | Eager JSON Baseline | Binary Span Tree | Tiered Hybrid |
|---|:--:|:--:|:--:|
...
`CosmosDiagnostics` (plus Fabian's dedup/aggregation work) maps directly onto the Tiered Hybrid's
summary, and `Activity`/`ActivitySource` gives an Eager JSON Baseline equivalent for free. Roll out
opt-in (keep current diagnostics, add the new format behind a verbosity flag, stabilize, then flip
the default per SDK).
...
- **Adopt:** the Tiered Hybrid (summary default) **+** the Binary Span Tree (binary detail tier).
  They already share the `AZD1` format, so this is one wire contract with two entry points: cheap
  aggregatable summary by default, full-fidelity binary on error/demand. This directly answers
  granularity, fan-out capture, binary+tool, and outcome-aware cost.
...
- **Stretch (not built):** an outcome-aware fast-path variant (the call's "Combo 4") — only if
  profiling shows the retained model itself is a hot-path cost; current numbers don't justify it.

---

## 10. Open questions for the scrum

1. **Canonical capture model / default verbosity** — adopt the Tiered Hybrid summary as default +
   the Binary Span Tree detail tier? Is per-operation auto-escalation on error the right rule?
