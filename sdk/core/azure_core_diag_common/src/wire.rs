...
//! The shared diagnostics **wire model** and a compact hand-rolled binary codec.
//!
//! The workspace has neither `rmp-serde` nor `bincode`, so rather than add an external
//! dependency for the spike we hand-roll a small, fully-controlled binary format. It is
//! deterministic (same input always yields the same bytes), self-describing via a
//! magic + version header, and optionally DEFLATE-compressed.
//!
//! Both Combo 1 (arena) and Combo 2 (detailed view) project their internal representation
//! into a [`WireTree`] and reuse [`encode`] / [`decode`]. This keeps a single binary
//! contract that the `diag-decode` tool (and a future cross-SDK decoder) targets.
//!
//! # Wire format (`AZD1`)
//!
//! ```text
...
/// Magic bytes identifying an Azure diagnostics blob, version 1.
...
///
/// Tuned so small point operations (S1–S3) skip DEFLATE entirely — compressing a ~300 byte blob
/// costs far more CPU than it saves — while wide fan-out (S4) crosses the threshold and benefits.
/// This is the single compression CPU/size tuning knob.
...
pub struct WireNode {
    ...
    #[serde(default)]
    pub kind: u8,
    /// Start tick (nanoseconds, from the mock clock).
    pub start_ns: u64,
...
pub enum DecodeError {
    ...
    BadMagic,
    /// The blob version is not supported by this decoder.
