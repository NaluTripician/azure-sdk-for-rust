...
mod live {
    ...
    async fn connect() -> Option<Connected> {
        ...
        let opts = DriverOptions::builder(account.clone())
            ...
            .build();
        match tokio::time::timeout(CONNECT_TIMEOUT, runtime.create_driver(opts)).await {
            Ok(Ok(_driver)) => {
    ...
    async fn run_scenarios(
        ...
        let plain = plain_runtime().await?;
        let driver = driver_for(&plain, &conn.account, Vec::new()).await?;
    ...
    /// Builds a plain runtime. Fault-injection rules are now applied per-driver via
    /// [`DriverOptionsBuilder::with_fault_injection_rules`], not on the runtime.
    async fn plain_runtime() -> Result<Arc<CosmosDriverRuntime>, CosmosError> {
        CosmosDriverRuntime::builder().build().await
    }
    ...
    async fn driver_for(
        ...
        let mut builder = DriverOptions::builder(account.clone())
            .with_capture_diagnostics_policy(DiagnosticsPolicy::always());
        if !rules.is_empty() {
            builder = builder.with_fault_injection_rules(rules)?;
        }
        runtime.create_driver(builder.build()).await
    }

    /// Reads the seeded item through a fresh driver carrying the given fault rules.
    async fn read_with_rules(
        ...
        let driver = driver_for(&runtime, &conn.account, rules).await?;
    ...
    async fn run_hedging_scenario(conn: &Connected, container: &ContainerReference) {
        ...
        let op_options = OperationOptionsBuilder::new()
            ...
            .build();
        let runtime = match plain_runtime().await {
            Ok(r) => r,
            Err(e) => {
        ...
        let driver = match driver_for(&runtime, &conn.account, vec![delay_rule()]).await {
            Ok(d) => d,
            Err(e) => {
    ...
    async fn cleanup(conn: &Connected, db_name: &str) -> Result<(), CosmosError> {
        let runtime = plain_runtime().await?;
        let driver = driver_for(&runtime, &conn.account, Vec::new()).await?;
        let db_ref = DatabaseReference::from_name(conn.account.clone(), db_name.to_string());
