...
//!
//! let rendered = azure_data_cosmos_driver::diagnostics::finish(rec, &policy);
...
/// Whether an operation is a point operation or not, selecting the gate's latency threshold.
///
/// Point operations (e.g. read/create/replace/delete a single item by id + partition key) use the
/// point latency threshold; everything else (queries, batch, feed reads) uses the non-point one.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum OperationKind {
    /// A single-item point operation.
    Point,
    /// A non-point operation (query, batch, feed).
    NonPoint,
}
