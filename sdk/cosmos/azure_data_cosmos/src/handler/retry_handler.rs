...
///
/// Reads [`METADATA_DETACHED_HARD_DEADLINE_ENV`] when set to a positive integer,
/// otherwise uses [`DEFAULT_METADATA_DETACHED_HARD_DEADLINE_SECS`]. The result is
/// clamped to [`MAX_METADATA_DETACHED_HARD_DEADLINE_SECS`] so that constructing the
/// timer can never overflow or leak unbounded background work.
fn metadata_detached_hard_deadline() -> Duration {
    let secs = std::env::var(METADATA_DETACHED_HARD_DEADLINE_ENV)
        .ok()
...
impl RetryHandler for BackOffRetryHandler {
        ...
        Fut: std::future::Future<Output = azure_core::Result<RawResponse>> + Send + 'static,
    {
        ...
        // cross-region failover decision (issue #4253). Data-plane requests keep
        // the in-line loop so that a dropped caller still cancels in-flight work
        // (important for non-idempotent writes).
        if request.resource_type.is_meta_data() {
            send_detached(
                retry_policy,
                request,
...
mod tests {
    ...
    /// Mirrors the .NET clamp fix: the default applies when the env var is absent,
    ...
    ///
    /// Kept as a single test because it mutates a process-global environment variable;
    /// splitting it would race with itself under cargo's parallel test runner.
    #[test]
    fn hard_deadline_resolution_and_clamping() {
        std::env::remove_var(METADATA_DETACHED_HARD_DEADLINE_ENV);
        assert_eq!(
            metadata_detached_hard_deadline().whole_seconds() as u64,
            DEFAULT_METADATA_DETACHED_HARD_DEADLINE_SECS,
            "absent env var should use the default"
        );

        // 60 days, well over the 24h clamp.
        std::env::set_var(METADATA_DETACHED_HARD_DEADLINE_ENV, "5184000");
        assert_eq!(
            metadata_detached_hard_deadline().whole_seconds() as u64,
            MAX_METADATA_DETACHED_HARD_DEADLINE_SECS,
            "an oversized env value must be clamped to the 24h maximum"
        );

        // Zero is not a usable deadline; fall back to the default.
        std::env::set_var(METADATA_DETACHED_HARD_DEADLINE_ENV, "0");
        assert_eq!(
            metadata_detached_hard_deadline().whole_seconds() as u64,
            DEFAULT_METADATA_DETACHED_HARD_DEADLINE_SECS
        );
        ...
        std::env::set_var(METADATA_DETACHED_HARD_DEADLINE_ENV, "not-a-number");
        assert_eq!(
            metadata_detached_hard_deadline().whole_seconds() as u64,
            DEFAULT_METADATA_DETACHED_HARD_DEADLINE_SECS
        );
